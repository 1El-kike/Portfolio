    ...
    return (
        <>
            ...
            <div className='container' style={{ marginBottom: '50px', marginTop: '60px' }}  >
                                        ...
                                        {arreglo.map(user => {
                                            return (
                                                <tr className='table-light' key={user.id}>
                                                    ...
                                                    <td><button onClick={() => openModal(2, user.id, user.actividad, user.responsable, user.observacion)} data-bs-toggle='modal' data-bs-target='#modalProducts' className='btn btn-warning'>
                                                        <i className='fa-solid fa-edit'></i>
                                                    </button>
                                                        &nbsp;
                                                        <button onClick={() => deletelist(user.id, user.name, user.actividad, user.responsable, user.observacion)} className='btn btn-danger'>
                        ...
                        <div className='modal-content'>
                            ...
                            <div className='modal-body'>
                                ...
                                <div className='input-group mb-3'>
                                    ...
                                    <input type='text' id='nombre' className='form-control' placeholder='Actividad' value={name} onChange={(e) => setname(e.target.value)}></input>
                                </div>
                                <div className='input-group mb-3'>
                                    <span className='input-group-text'><i className='fa-solid fa-comment'></i></span>
                                    <input type='text' id='responsable' className='form-control' placeholder='Responsable' value={responsable} onChange={(e) => setresponsable(e.target.value)}></input>
                                </div>
                                <div className='input-group mb-3'>
                                    <span className='input-group-text'><i className='fa-solid fa-bell'></i></span>
                                    <input type='text' id='observaciones' className='form-control' placeholder='Observaciones' value={observaciones} onChange={(e) => setobservaciones(e.target.value)}></input>
                                </div>

                            </div>
                            <div className='modal-footer'>
                                <div className='d-grid col-5 mx-auto'>
                                    <button onClick={() => addlist(arreglo.length, name, responsable, observaciones)} className='btn btn-success gap-2'>
                ...
                <button onClick={() => openModal(1)} className='btn btn-success' data-bs-toggle='modal' data-bs-target='#modalProducts' ><i className='fa-solid fa-circle'></i> Agregar</button>
            </div >
        </>
    )
}