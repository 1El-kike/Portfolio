...
import { faFolderClosed, faHandsHelping, faTools } from '@fortawesome/free-solid-svg-icons'
            ...
            <UserContext.Provider value={val}>
                ...
                <div className='container  m-3 ' style={{ height: "auto" }}>
                    <div className='d-flex row  justify-content-center h-100 '>
                        <div className='col-12 rounded m-4' style={{ height: 'auto', background: "radial-gradient(ellipse at top,rgb(12,138,160),transparent),radial-gradient(ellipse at bottom,rgb(107,0,123),transparent)" }}>

                            <CardEdixts
                                link={[tools, equipo, delet, help]}
                                title={elementos.map((ele, item) => <TabsTitle cantidad={ele} onClick={() => result(item)} setvalor={setval} />)}
                                texto={elem}
                            />
                            <Alerts
                                heading={"Administrator"}
                                texthead={"Aqui podra designar hacia donde quieren dirigir la informacion"}
                                textbody={"Seleccione las diferentes opciones en la primera casilla que se muestran anteriormente"}
                            />
                        </div>
